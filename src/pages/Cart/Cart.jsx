...
import { clearItems, selectorCart } from '../../redux/slices/cart/cartSlice';

import cl from './Cart.module.scss';

const Cart = () => {
  const { items, totalCount, totalPrice } = useSelector(selectorCart);
  const dispatch = useDispatch();

  const onClearItems = () => {
    if (window.confirm('Вы действительно хотите полностью очистить корзину')) {
      dispatch(clearItems());
    }
  };
      ...
      {totalCount > 0 ? (
        <div className={cl.cart}>
          <div className={cl.heading}>
            ...
            <div className="hovered" onClick={onClearItems}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
                height="20"
          ...
          <div className={cl.footer}>
            ...
            <div className={cl.footer__bottom}>
              ...
              <button className={cl.btn + ' ' + cl.btn__primary}>Оплатить сейчас</button>
            </div>
          </div>
        </div>
      ) : (
        <NotFoundError title="Ваша корзина пуста, пожалуйста, вернитесь на главную и добавьте товары в вашу коризну" />
      )}
    </main>
  );
};
